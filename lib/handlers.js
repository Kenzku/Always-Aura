...
handlers[protocol.TYPE_ID_SUBSCRIBE] = function(client, args) {
  ...
  this.topics[topicUri][client.id] = true;
    // if topic is room:lightStatus
    if (topicUri == Constant.room.topURI.lightStatus){
        var msg = JSON.stringify([protocol.TYPE_ID_EVENT,topicUri,[Constant.room.subscribe.lightStatus, client.id]]);
        this.broadcast(topicUri,msg,false); // this is the "Server", server.js
    }else{
        var msg = JSON.stringify([protocol.TYPE_ID_EVENT,topicUri,["NORMAL",client.id]]);
...
handlers[protocol.TYPE_ID_UNSUBSCRIBE] = function(client, args) {
    ...
    if (topicUri) {
        ...
        var msg = JSON.stringify([protocol.TYPE_ID_EVENT,topicUri,["unsubscribe","session id",client.id]]);
        this.broadcast(topicUri,msg,false);
    } else {
